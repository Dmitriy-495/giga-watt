...
func load(ctx context.Context, tx pgx.Tx, f *excelize.File) error {
	...
	steps := []struct {
		...
		{"users", []string{"email", "is_superuser"}, l.loadUsers},
...
func printWarnings(warnings []string) {
	...
	fmt.Println("fixtures: предупреждения:")
...
func printSummary(counts map[string]int) {
	...
	order := []string{
		...
		"employee_emails", "users", "staff_positions", "operational_objects",
	}
...
func (l *loader) loadUsers(s *sheet) error {
	for i, row := range s.rows {
		email := s.get(row, "email")
		if email == "" {
			return fmt.Errorf("users строка %d: пустой email", s.rowNumber(i))
		}

		isSuperuser := parseBool(s.get(row, "is_superuser"))

		rows, err := l.tx.Query(l.ctx, `
			SELECT DISTINCT employee_id
			FROM employee_emails
			WHERE email = $1
			ORDER BY employee_id`, email)
		if err != nil {
			return fmt.Errorf("users строка %d (%s): поиск Employee по employee_emails: %w",
				s.rowNumber(i), email, err)
		}
		...
		for rows.Next() {
			...
			if err := rows.Scan(&employeeID); err != nil {
				rows.Close()
				return fmt.Errorf("users строка %d (%s): чтение employee_id: %w",
					s.rowNumber(i), email, err)
			}
		...
		if err := rows.Err(); err != nil {
			rows.Close()
			return fmt.Errorf("users строка %d (%s): чтение employee_emails: %w",
				s.rowNumber(i), email, err)
		}
		rows.Close()

		switch len(employeeIDs) {
		case 0:
			l.warnings = append(l.warnings, fmt.Sprintf(
				"users строка %d (%s): сотрудник с указанным email не найден",
				s.rowNumber(i), email))
		case 1:
			// Employee однозначно определён через employee_emails.
		case 2:
			l.warnings = append(l.warnings, fmt.Sprintf(
				"users строка %d (%s): неоднозначное соответствие — найдено несколько сотрудников",
				s.rowNumber(i), email))
		default:
			l.warnings = append(l.warnings, fmt.Sprintf(
				"users строка %d (%s): неоднозначное соответствие — найдено сотрудников: %d",
				s.rowNumber(i), email, len(employeeIDs)))
		}

		_, err = l.tx.Exec(l.ctx, `
			INSERT INTO users (email, is_superuser)
			VALUES ($1, $2)`, email, isSuperuser)
		if err != nil {
			return fmt.Errorf("users строка %d (%s): %w", s.rowNumber(i), email, err)
		}
	}
