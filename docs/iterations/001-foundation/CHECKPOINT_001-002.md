...
По запросу пользователя технический долг был пересмотрен после первого
прохода. Закрыто в этом же CHECKPOINT:

- **Down-миграции** — добавлены для всех 6 миграций
  (`*.down.sql`). Проверен полный (`up` → `down -all` → `up`) и частичный
  (`down 1` конкретно для `000006`) цикл: после `down` БД остаётся только
  с `schema_migrations`, ни одной пользовательской таблицы/типа; после
  повторного `up` — идентичное чистое состояние. Отдельно подтверждено,
  что `down` миграции `000006` корректно восстанавливает поведение
  `000003` (уязвимость снова воспроизводима на этой версии — это и
  требовалось от точного отката).
- **Explicit cycle-check** — добавлен regression-тест
  (`TestHierarchy_CycleWalkCatchesPreexistingCycle`), который временно
  отключает триггер, создаёт цикл напрямую в обход валидации, включает
  триггер обратно и проверяет, что тривиальная запись (без смены
  type/parent_id) в уже циклические строки всё равно отклоняется —
  именно независимой проверкой цикла, а не children-block.
- **CI** — добавлен `.github/workflows/ci.yml`: backend-job (Postgres
  service, build/vet/gofmt, миграции, `go test`, загрузка fixtures) и
  frontend-job (`npm ci` + `nuxt build`). Не запущен на самом GitHub
  (недоступно из этой сессии) — но каждый отдельный шаг воркфлоу выполнен
  вручную в этой же сессии и подтверждённо работает.

Дополнительно **обнаружен и задокументирован** (не введён заново) факт:
реальное минимальное требование к тулчейну Go — **1.25**, а не 1.22.3
из ADR-0004. Причина — предсуществующая (с Iteration 000) транзитивная
тестовая зависимость `cleanenv → gopkg.in/yaml.v3 → kr/pretty →
rogpeppe/go-internal`; `go.sum` фиксировал `go-internal v1.16.0` ещё в
первом коммите Bootstrap, просто директива `go` в `go.mod` не была
приведена в соответствие. `backend/go.mod` теперь честно указывает
`go 1.25.0` и `toolchain go1.25.13`; addendum добавлен в ADR-0004.
Excel-библиотека понижена до `excelize v2.9.0` (требует только Go 1.18)
— это не отменяет находку, но не усугубляет её сверх того, что уже было.

Сознательно оставлено пользователю на отдельное обсуждение (требуют
предметных/архитектурных решений, а не механической работы):

- `organization_units` без `DELETE` — нужно сначала решить правило для
  зависимых записей (RESTRICT/архивирование/каскад).
- Frontend остаётся read-only (без форм создания/редактирования) — нужно
  отдельно решить объём и стек (сейчас нет UI-библиотеки).
